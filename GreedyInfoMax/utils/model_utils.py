...
def distribute_over_GPUs(opt, model, num_GPU):
    ...
    logging.info("Let's use " + str(num_GPU) + " GPUs!")
...
def makeDeltaOrthogonal(weights, gain):
    ...
    cols = weights.size(1)
    if rows > cols:
        logging.info("In_filters should not be greater than out_filters.")
...
def reload_weights(opt, model, optimizer, reload_model):
    ...
    if reload_model:  # or opt.model_type == 2)
        logging.info("Loading weights from " + opt.model_path)
    ...
    # reload weights and optimizers for continuing training
        ...
        logging.info("Continuing training from epoch " + opt.start_epoch)
