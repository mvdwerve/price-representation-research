...
def fit_PCA_and_plot(opt, feature_space, speaker_labels, label, clf=None):
    feature_space = np.reshape(
        feature_space, (np.shape(feature_space)[0] * np.shape(feature_space)[1], -1)
    )
    # print(feature_space.shape)
    ...
    if clf is None:
        clf = PCA(n_components=2, whiten=True).fit(feature_space)

    # X: array, shape(n_samples, n_features)
    projection = clf.transform(feature_space)
